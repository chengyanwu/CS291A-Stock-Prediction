...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in: int, target_dim: int, patch_len: int, stride: int, num_patch: int,
                 ...
                 y_range: Optional[tuple] = None, verbose: bool = False, cross_attention=False, **kwargs):
        ...
        self.backbone = PatchTSTEncoder(c_in, num_patch=num_patch, patch_len=patch_len,
                                        ...
                                        pe=pe, learn_pe=learn_pe, verbose=verbose, cross_attention=cross_attention, **kwargs)
    ...
    def forward(self, z):
        ...
        """
        z = self.backbone(
            z)                                                                # z: [bs x nvars x d_model x num_patch]
        z = self.head(z)
...
class PredictionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.individual:
            x_out = []
            for i in range(self.n_vars):
                # z: [bs x d_model * num_patch]
                z = self.flattens[i](x[:, i, :, :])
                # z: [bs x forecast_len]
                z = self.linears[i](z)
                z = self.dropouts[i](z)
                x_out.append(z)
            # x: [bs x nvars x forecast_len]
...
class PretrainHead(nn.Module):
    ...
    def forward(self, x):
        ...
        # [bs x nvars x num_patch x d_model]
        x = x.transpose(2, 3)
        # [bs x nvars x num_patch x patch_len]
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len,
                 ...
                 pe='zeros', learn_pe=True, verbose=False, cross_attention=False, **kwargs):
        ...
        if not cross_attention:
            ...
            self.W_pos = positional_encoding(
                pe, learn_pe, c_in * num_patch, d_model)
    ...
    def forward(self, x) -> Tensor:
        ...
        if not self.cross_attention:
            ...
            u = torch.reshape(x, (bs, num_patch * n_vars, self.d_model))
            # u = self.dropout(u + self.W_pos.repeat(n_vars, 1))
