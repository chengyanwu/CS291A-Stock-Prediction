    ...
    def before_fit(self):
        if not self.steps_per_epoch:
            self.steps_per_epoch = (
                len(self.dls.train) if not self.dls.stocks else self.dls.train_len)
...
class LRFinderCB(Callback):
    ...
    def smoothing(self, beta):
        ...
        self.aver_loss = beta * self.aver_loss + \
            (1-beta) * self.loss.detach().item()
