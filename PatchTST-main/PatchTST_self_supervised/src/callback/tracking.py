...
class TrackTimerCB(Callback):
    ...
    def after_epoch_train(self):
        self.learner.epoch_time = self.format_time(
...
class TrackTrainingCB(Callback):
    ...
    def initialize_recorder(self):
        ...
        for name in self.metric_names:
            if self.train_metrics:
                recorder['train_'+name] = []
            if self.valid_metrics:
                recorder['valid_'+name] = []
        self.recorder = recorder
        self.learner.recorder = recorder

    def initialize_batch_recorder(self, with_metrics):
        batch_recorder = {'n_samples': [],
                          'batch_losses': [], 'with_metrics': with_metrics}
    ...
    def compute_scores(self):
        ...
        self.n = sum(self.batch_recorder['n_samples'])
        try:
            values['loss'] = sum(
                self.batch_recorder['batch_losses']).item()/self.n  # averaging
        except AttributeError:
            values['loss'] = sum(self.batch_recorder['batch_losses'])/self.n
...
class PrintResultsCB(Callback):
    ...
    def before_fit(self):
        if self.run_finder:
            return          # don't print if lr_finder is called
        if not hasattr(self.learner, 'recorder'):
            return      # don't print if there is no recorder
...
class SaveModelCB(TrackerCB):
    ...
    def _save(self, fname, path):
        if self.global_rank == self.save_process_id:
            self.last_saved_path = self.learner.save(
                fname, path, with_opt=self.with_opt)
    ...
    def after_epoch(self):
        ...
        if self.new_best:
            ...
            if self.impatient_level > self.patient:
                print(
                    f'No improvement since epoch {self.epoch-self.impatient_level}: early stopping')
                raise KeyboardInterrupt
