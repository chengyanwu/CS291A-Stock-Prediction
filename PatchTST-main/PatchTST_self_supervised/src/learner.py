...
class Learner(GetAttr):
    ...
    def predict(self, test_data, weight_path=None, Dataset=None, Dataloader=None, batch_size=None):
        ...
        cb = GetPredictionsCB()
        self.add_callback(cb)
        test_dl = self._prepare_data(
            test_data, Dataset, Dataloader, batch_size)
    ...
    def fine_tune(self, n_epochs, base_lr=None, freeze_epochs=1, pct_start=0.3):
        ...
        """
        assert (n_epochs > 0) | (freeze_epochs >
                                 0), "Either n_epochs or freeze_epochs has to be > 0"
        if not base_lr:
            base_lr = self.lr
        ...
        if freeze_epochs > 0:
            ...
            self.freeze()
            self.fit_one_cycle(freeze_epochs, lr_max=base_lr,
                               pct_start=pct_start)
    ...
    def lr_finder(self, start_lr=1e-7, end_lr=10, num_iter=100, step_mode='exp', show_plot=True, suggestion='valley'):
        ...
        """
        n_epochs = num_iter//len(self.dls.train) + \
            1 if not self.dls.stocks else num_iter//self.dls.train_len + 1
        # indicator of lr_finder method is applied
        self.run_finder = True
        # add LRFinderCB to callback list and will remove later
        cb = LRFinderCB(start_lr, end_lr, num_iter,
                        step_mode, suggestion=suggestion)
        # fit
...
def get_layer_output(inp, model, layers=None, unwrap=False):
    ...
    h_list = [getattr(model, layer).register_forward_hook(
        getActivation(layer)) for layer in layers]
