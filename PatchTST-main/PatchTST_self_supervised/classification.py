...
parser.add_argument('--context_points', type=int,
                    default=336, help='sequence length')
...
print('args:', args)
args.save_model_name = 'patchtst_supervised'+'_cw'+str(args.context_points)+'_tw'+str(args.target_points) + '_patch'+str(
    args.patch_len) + '_stride'+str(args.stride)+'_epochs'+str(args.n_epochs) + '_model' + str(args.model_id)
args.save_path = 'saved_models/' + args.dset + \
    '/patchtst_supervised/' + args.model_type + '/'
...
def find_lr():
    ...
    # define learner
    learn = Learner(dls, model, loss_func, cbs=cbs, lr=args.lr, classification=True)
...
def linear_probe_func(lr=args.lr):
    ...
    df.to_csv(args.save_path + args.model_id +
...
def test_func():
    ...
    cbs = [RevInCB(dls.vars)] if args.revin else []
    cbs += [PatchCB(patch_len=args.patch_len, stride=args.stride)]
    learn = Learner(dls, model, cbs=cbs, classification=True)
